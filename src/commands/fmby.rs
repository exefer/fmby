...
use crate::constants::{FMHY_SINGLE_PAGE_ENDPOINT, FmhyChannel};
use crate::db::{ChunkSize, infer_wiki_url_status};
use crate::entities::enums::WikiUrlStatus;
use crate::entities::{prelude::*, wiki_urls};
use crate::message::get_content_or_referenced;
use crate::url::{clean_url, extract_urls};
use crate::wiki::collect_wiki_urls;
...
async fn fmby(_ctx: Context<'_>) -> Result<(), Error> {
    Ok(())
}

#[derive(poise::ChoiceParameter)]
enum OnlineStatusChoice {
    Online,
    Idle,
    DoNotDisturb,
    Invisible,
...
async fn status(ctx: Context<'_>, status: OnlineStatusChoice) -> Result<(), Error> {
...
async fn search(
    ...
    let results = crate::wiki::search_in_wiki(&query).await.unwrap();
...
async fn autocomplete_url<'a>(
    ...
    let urls: Vec<String> = WikiUrls::find()
        ...
        .apply_if((!partial.is_empty()).then_some(()), |query, _| {
            query.filter(wiki_urls::Column::Url.ilike(format!("%{}%", clean_url(partial))))
        })
        .limit(25)
...
async fn inconsistencies(ctx: Context<'_>) -> Result<(), Error> {
